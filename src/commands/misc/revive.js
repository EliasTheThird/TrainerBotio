...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    const userPing = `<@${interaction.user.id}>`; // Ping the user who used the command
    const rolePing = `<@&${roleId}>`;

    const messageContent = `${rolePing}, ${userPing} would like to chat! - ${description}`;

    await interaction.reply({ content: messageContent, allowedMentions: { parse: ['roles', 'users'] } });
  },
  name: 'revive',
